...
import { Trash2, Plus, Minus, ArrowRight, Tag, Truck, CheckCircle, AlertCircle, Loader } from 'lucide-react';
import Button from '../components/ui/Button';
import { 
  fetchCart, addToCart, updateCartItem, removeCartItem, 
  validatePromoCode, fetchShippingRates, getShippingRate,
...
const Cart = () => {
  ...
  const discount = promoData ? (promoData.discount_type === 'percentage' ? (subtotal * promoData.discount_value / 100) : promoData.discount_value) : 0;
  const total = subtotal - discount + shippingCost;

  if (loading) return <div className="cart-loading container"><Loader className="spinner" /><p>Chargement du panier...</p></div>;
  ...
  if (orderSuccess) {
    return (
      <div className="cart-success container fade-in">
        ...
        <p>Merci pour votre confiance. Votre numéro de commande est : <strong>{orderSuccess}</strong></p>
        <Link href="/shop"><Button variant="primary" className="mt-4">Continuer mes achats</Button></Link>
      </div>
    );
  }
        ...
        <div className="cart-layout">
              ...
              <div key={item.id} className="cart-item">
                ...
                <div className="cart-item-details">
                  <div className="item-header">
                     ...
                     <p className="item-price">{(item.price * item.quantity).toLocaleString()} DA</p>
                  </div>
                  <Link href={`/product/${item.product_id}`} className="item-title">{item.product_title}</Link>
          ...
          <div className="cart-summary">
            ...
            <div className="summary-section">
              <label><Truck size={16} /> Livraison (Wilaya)</label>
              <select value={selectedWilaya} onChange={handleWilayaChange} className="wilaya-select">
                <option value="">Sélectionnez votre wilaya</option>
                {wilayas.map(w => (
                  <option key={w.wilaya_code} value={w.wilaya_code}>{w.wilaya_code} - {w.wilaya_name}</option>
                ))}
              </select>
            </div>

            <div className="summary-section">
              <label><Tag size={16} /> Code Promo</label>
              ...
              {promoError && <p className="promo-error"><AlertCircle size={12} /> {promoError}</p>}
              {promoData && <p className="promo-success"><CheckCircle size={12} /> Réduction de {promoData.discount_value}{promoData.discount_type === 'percentage' ? '%' : ' DA'} appliquée</p>}
            </div>
            ...
            <div className="summary-row">
              <span>Livraison</span>
              <span>{shippingCost > 0 ? `${shippingCost.toLocaleString()} DA` : '--'}</span>
            </div>
